...
import useGetCourseById, { type ParsedCourseData } from '@/hooks/useGetCourseById';
...
const Form = ({ course, allUsers }: { course: ParsedCourseData; allUsers: DatabaseUser[] }) => {
  ...
  const form = useForm({
    ...
    defaultValues: {
      ...
      endDate: course?.endDate ? new Date(course.endDate) : new Date(new Date().setDate(new Date().getDate() + 10)),
      imageURL: course?.imageURL || '',
      students: course?.students?.map((student) => student.id.toString()) || [],
      teacher: course?.teacher?.id.toString() || '',
    },
  });
  ...
  const { availableStudents, availableTeachers } = allUsers.reduce(
    (
      ...
      if (user.courseId === null || user.courseId === course.id) {
                  ...
                  <Combobox
                    ...
                    <ComboboxChips>
                      <ComboboxChipsInput
                        ...
                        value={
                          field.value
                            ? availableTeachers[field.value]?.firstName + ' ' + availableTeachers[field.value]?.lastName
                            : ''
                        }
                        placeholder=""
                      />
...
export default function EditCourse() {
  ...
  const { data: allUsers, isLoading: allUsersLoading, error: allUsersError } = useGetUsers();

  const isLoading = courseLoading || allUsersLoading;
  const error = courseError || allUsersError;
  ...
  return (
    <div className="py-2">
      <h1 className="text-2xl font-bold pb-6">Edit Course</h1>
      <Form course={course} allUsers={allUsers} />
    </div>
  );
}
