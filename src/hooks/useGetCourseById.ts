...
export interface ParsedCourseData extends Course {
...
const useGetCourseById = ({
  ...
  courseid: number;
  userId?: string;
}): ReturnType<typeof useQuery<ParsedCourseData>> => {
  return useQuery({
    queryKey: ['courseById', userId, courseid],
    queryFn: async () => {
    ...
    enabled: !!userId && Number.isInteger(courseid), // Only run the query if userId is provided
  });
};
